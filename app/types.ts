...
import { 
  ...
  Plus, MousePointerSquare, CheckCircle2, LayoutTemplate
} from "lucide-react";
...
export interface BoxPartConfig {
  ...
  type: PartType;
  name: string;
  customHeight?: number; // Alto de solapa/tapa
  customMargin?: number; // Holgura
  customWidth?: number;  // Para anchos fijos como el portacafé
  customLength?: number; // Para largos fijos
  targetProductIds: string[]; // Productos a los que aplica (ej. Tapa interna)
}
...
export interface CutResult {
  ...
  flapSize: number;
  colorClass: string;
  isStrip?: boolean; // Para el agarrador
}
...
interface SavedBox {
  ...
  boxParts: BoxPartConfig[];
  // Legacy (para migración)
  boxType?: string; heightMargin?: string; lidHeight?: string; lidMargin?: string;
}
...
const ICON_MAP: Record<string, any> = {
  donut: Circle, dessert: Dessert, coffee: Coffee, cookie: Cookie,
  juice: CupSoda, sandwich: Sandwich, default: Box
};
...
const PART_COLORS: Record<PartType, string> = {
  BASE: 'text-blue-500',
  FULL_LID: 'text-purple-500',
  INTERNAL_LID: 'text-emerald-500',
  HOLDER: 'text-amber-500',
  HANDLE: 'text-pink-500'
};
...
export function BoxCalculator() {
  ...
  // Estados de Configuración de Caja
  const [rows, setRows] = useState(3);
  const [cols, setCols] = useState(5);
  const [placedProducts, setPlacedProducts] = useState<PlacedProduct[]>([]);
  
  // MOTOR DE PIEZAS MODULAR
  const [boxParts, setBoxParts] = useState<BoxPartConfig[]>([
    { id: 'base-initial', type: 'BASE', name: 'Base Estuche', customMargin: 1.5, targetProductIds: [] }
  ]);
  ...
  // Persistencia
  useEffect(() => {
    const storedT = localStorage.getItem("box_templates_v4");
    const storedB = localStorage.getItem("saved_boxes_v4");
    if (storedT) setProductTemplates(JSON.parse(storedT));
    else { setProductTemplates(DEFAULT_PRODUCTS); localStorage.setItem("box_templates_v4", JSON.stringify(DEFAULT_PRODUCTS)); }
    if (storedB) setSavedBoxes(JSON.parse(storedB));
  }, []);

  useEffect(() => {
    localStorage.setItem("box_templates_v4", JSON.stringify(productTemplates));
    localStorage.setItem("saved_boxes_v4", JSON.stringify(savedBoxes));
  ...
  // Cargar Caja (con migración automática de la versión anterior)
  const loadBox = (box: SavedBox) => {
    ...
    // Si es una caja guardada con la versión anterior (tiene boxType), la migramos al nuevo formato
    if (box.boxType) {
      const migratedParts: BoxPartConfig[] = [
        { id: `base-${Date.now()}`, type: 'BASE', name: 'Base Estuche', customMargin: parseFloat(box.heightMargin || "1.5"), targetProductIds: [] }
      ];
      if (box.boxType === 'with-lid') {
        migratedParts.push({ id: `lid-${Date.now()}`, type: 'FULL_LID', name: 'Tapa Completa', customHeight: parseFloat(box.lidHeight || "3"), customMargin: parseFloat(box.lidMargin || "0.3"), targetProductIds: [] });
      } else if (box.boxType === 'internal-half-lid') {
        migratedParts.push({ id: `int-${Date.now()}`, type: 'INTERNAL_LID', name: 'Tapa Interna', customMargin: -0.2, targetProductIds: box.placedProducts.map(p => p.instanceId) });
      }
      setBoxParts(migratedParts);
    } else {
      setBoxParts(box.boxParts);
    }
    setSelectionMode({ active: false, partId: null });
  };

  // 1. Cálculo de Dimensiones Internas Puras
  ...
  // 2. Motor de Piezas (El núcleo de la escalabilidad)
  useEffect(() => {
    ...
    boxParts.forEach(part => {
      if (part.type === 'BASE') {
        partsToCut.push({
          partId: part.id, name: part.name, type: part.type,
          cutWidth: totalInnerWidth + 2 * innerHeight,
          cutLength: totalInnerLength + 2 * innerHeight,
          flapSize: innerHeight, colorClass: PART_COLORS[part.type]
        });
      } 
      else if (part.type === 'FULL_LID') {
        const flap = part.customHeight || 3;
        const margin = part.customMargin || 0.3;
        partsToCut.push({
          partId: part.id, name: part.name, type: part.type,
          cutWidth: (totalInnerWidth + margin) + 2 * flap,
          cutLength: (totalInnerLength + margin) + 2 * flap,
          flapSize: flap, colorClass: PART_COLORS[part.type]
        });
      }
        ...
        // Si hay objetivos seleccionados, el ancho es la suma de las columnas que ocupan
        if (targets.length > 0) {
        ...
        // Largo: total interno menos 2mm (1mm por lado) = -0.2
        const cutL = totalInnerLength - 0.2;
        // Alto de solapa: alto base menos 1mm = -0.1
        const flap = innerHeight - 0.1;

        partsToCut.push({
          partId: part.id, name: part.name, type: part.type,
          cutWidth: boxW + 2 * flap, // Ancho de los productos seleccionados + solapas
          cutLength: cutL + 2 * flap,
          flapSize: flap, colorClass: PART_COLORS[part.type]
        });
      }
      else if (part.type === 'HOLDER') {
        const flap = Math.min(innerHeight - 0.1, part.customHeight || 5.9);
        const w = part.customWidth || 10;
        const l = part.customLength || 10;
        partsToCut.push({
          partId: part.id, name: part.name, type: part.type,
          cutWidth: w + 2 * flap,
          cutLength: l + 2 * flap,
        ...
        // Tira recta: Ancho igual a la base, largo igual a la base + 23cm por lado (46cm total)
        const stripL = 46 + totalInnerLength;
        partsToCut.push({
          partId: part.id, name: part.name, type: part.type,
          cutWidth: totalInnerWidth,
          cutLength: stripL,
          flapSize: 0, colorClass: PART_COLORS[part.type], isStrip: true
        });
      }
    });
  ...
  // 3. Resumen de Producción Dinámico
  const productionSummary = useMemo(() => {
    ...
    const partsData = results.partsToCut.map(cut => {
      ...
      totalSheets += sheets;
      return { ...cut, perSheet, sheets, unitCost };
    });

    const totalProjectCost = totalSheets * sheetPrice;
    const totalUnitCost = partsData.reduce((sum, p) => sum + p.unitCost, 0);

    return { partsData, totalSheets, totalProjectCost, totalUnitCost };
  }, [results, productionQuantity, sheetPrice]);

  // Interacción con el Grid (Modo normal o Modo Selección)
  const handleCellClick = (r_idx: number, c_idx: number) => {
    if (selectionMode.active) return; // No agregar productos si estamos seleccionando piezas
  ...
  const handleProductClick = (instanceId: string) => {
    if (selectionMode.active && selectionMode.partId) {
      setBoxParts(prev => prev.map(pt => {
        if (pt.id === selectionMode.partId) {
          const exists = pt.targetProductIds.includes(instanceId);
          return { ...pt, targetProductIds: exists ? pt.targetProductIds.filter(id => id !== instanceId) : [...pt.targetProductIds, instanceId] };
        }
        return pt;
      }));
    } else {
  ...
  const addPart = (type: PartType) => {
    const names: Record<PartType, string> = { BASE: 'Base', FULL_LID: 'Tapa', INTERNAL_LID: 'Tapa Interna', HOLDER: 'Portacafé', HANDLE: 'Agarrador' };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 md:p-8 bg-zinc-950 min-h-screen text-zinc-100 font-sans">
      <header className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 font-black tracking-[0.2em] uppercase">Motor Modular de Piezas</p>
          </div>
        </div>
        <div className="flex items-center gap-3 bg-zinc-900 p-2 rounded-2xl border border-zinc-800 shadow-xl">
          ...
          <button onClick={() => {
            ...
            setSavedBoxes([{ id: Date.now().toString(), name: currentBoxName, rows, cols, placedProducts, boxParts }, ...savedBoxes]);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        {/* COLUMNA IZQUIERDA (Herramientas y Producción) */}
            ...
            <div className="bg-zinc-900 p-8 rounded-[2rem] border border-zinc-800 shadow-xl space-y-6">
              <div className="flex justify-between items-center border-b border-zinc-800 pb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => {
                    const sel = document.getElementById('part-selector') as HTMLSelectElement;
                    addPart(sel.value as PartType);
                  }} className="bg-zinc-800 hover:bg-zinc-700 p-2 rounded-lg text-white transition-colors"><Plus size={16} /></button>
                </div>
              </div>

              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
                {boxParts.map((part, index) => (
                  <div key={part.id} className={`p-4 rounded-2xl border-2 transition-all ${selectionMode.partId === part.id ? 'border-emerald-500 bg-emerald-500/10' : 'border-zinc-800 bg-black/40'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {/* Inputs dinámicos dependiendo del tipo de pieza */}
                      {part.type === 'BASE' && (
